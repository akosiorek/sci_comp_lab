...
function worksheet2()
    ...
    for i = 1: length(funs) % Method Loop
        ...
        legend('dt=1  ', 'dt=1/2 ', 'dt=1/4 ', 'dt=1/8 ', 'dt=1/16', 'dt=1/32', 'precise');  % Add legend
    ...
    %Display stability 
    deltas_t={'dt=1  ', 'dt=1/2 ', 'dt=1/4 ', 'dt=1/8 ', 'dt=1/16', 'dt=1/32'}; % Display headers
    fprintf(strcat('\t\t', 'Stability', '\n'))
    names_t={'Exp.Euler\t', 'Heun     \t', 'Imp.Euler\t','A-M     \t', 'A-M L1  \t', 'A-M L2  \t'};
    fprintf(strcat('\t\t',names_t{:}, '\n\n')); 
    
    stability=stability';
    for i=1:length(deltas) % Build crosses array
        for j=1:length(funs)
        if (stability(i,j)==1)
            stability_t{i,j}='X\t\t';
        else
    ...
    format rat
        for i=1:length(deltas) % Display content
            fprintf(strcat(deltas_t{i},'\t\t',stability_t{i,:},'\n\n'));
        end
    format short
